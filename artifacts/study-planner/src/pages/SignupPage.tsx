...
export default function SignupPage() {
  ...
  const createInitialUserData = async (userId: string, name: string, email: string) => {
    // Create base profile
    ...
    const subjectRef = doc(db, 'subjects', `${userId}_default`);
  ...
  const onSubmit = async (data: SignupForm) => {
    ...
    try {
      ...
      await createInitialUserData(userCredential.user.uid, data.name, data.email);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute top-[20%] right-[-10%] w-[40rem] h-[40rem] bg-primary/5 rounded-full blur-[120px] pointer-events-none" />
